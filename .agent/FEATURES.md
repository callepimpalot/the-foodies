...
| Capture — Recipe URL | `netlify/functions/fetch-recipe.js` | TASK_08. Fetches a recipe blog server-side (CORS blocks the browser), reads `schema.org/Recipe` JSON-LD, maps it straight to the app's shape; falls back to page text → Gemini when there's no JSON-LD. |
| Capture — Social link (Instagram / TikTok / Shorts) | `FEATURE_social_link_capture.md` — `netlify/functions/fetch-recipe.js` (`isSocialPostUrl`, `buildSocialText`, `fetchSocialPost`) | Social posts have no JSON-LD and their HTML is a login shell, but the **caption** is usually the whole recipe. A third branch in the same function asks the VPS social-ingest service (`~/.hermes/recipe-ingest/server.py`) for caption + transcript and returns the **same shape as the page fallback**, so the client reuses `extractRecipe({ text })` — no new schema, no new screen. **Inert until `SOCIAL_INGEST_URL` + `SOCIAL_INGEST_TOKEN` are set in Netlify.** Caption path measured 2.1–2.9 s; transcription is ~16 s and deliberately off this path. (Sep 24) |
