...
export function ShopView() {
    ...
    const { items: essentialItems, toggleFlag } = useInventory();
    const [checkedKeys, setCheckedKeys] = useState(() => new Set());
    // Snapshot on mount so an item doesn't vanish mid-shop the instant it's
    // checked off — toggleFlag still fires immediately so Pantry stays in
    // sync even if the user leaves before finishing the rest of the list.
    const [householdSnapshot] = useState(() => essentialItems.filter((i) => i.flagged));
